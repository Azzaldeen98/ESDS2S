...
//                                                    delay((indexFlow * 100) as Long)
                                                    withContext(Dispatchers.Main) {
                                                        semaphore.withPermit {
                                                            ...
                                                            Log.d("CompletedExoPlayer", "$isComplated")
                                                            if(isComplated) {
                                                            isResponsed=true
...
//                                                            )
                                                ...
                                                semaphore.withPermit { isResponsed=true }
//                                                callBack?.onRequestIsSuccess2(stopExoPlayer)
                                            }
                                        }
                                    }
                                  ...
                                  if (value is File) {
                                      playAudio2(
                                          ...
                                          object : ICustomPlayerListener<ExoPlayer> {
                                              ...
                                              override fun onCompletionListener(mp: ExoPlayer?) {
//                                                  Log.d("onCompletionListener5", "$isComplated")
//                                                  if (isComplated) {
//                                                      Log.d("onCompletionListener5", "$isComplated")
//                                                      isResponsed = true
//                                                  }
...
//                                          callBack?.onRequestIsSuccess2(stopExoPlayer)
                                            ...
                                            Log.d("Completed", response!!)
                                                semaphore.withPermit {
                                                 isComplated = true
                                                    flowAudioFileMap?.put(indexFlow, "###")
                                                }
                                        }
                            ...
                            if (value is File) {
                                withContext(Dispatchers.Main) {
                                    semaphore?.withPermit {
                                        ...
                                        if(flowAudioFileMap?.containsValue("###")==true){
                                    ...
                                    delay(1000L)
                                    while (semaphore?.withPermit{ myExoPlayer?.isPlaying } == true || !isComplated) {
                                        delay(500L)
                                    }
                                }
                            } else if (value is String) {
                                    ...
                                    semaphore?.withPermit {
                                        ...
                                        playListener = playExoPlayer(
                                            ...
                                            @OptIn(UnstableApi::class)
                                                ...
                                                override fun onCompletionListener(mp: ExoPlayer?,isComplete:Boolean) {
                                                    Log.d("onCompletionListener", "$isComplete -- $index")
                                                    isComplated=true
                                                    if(isComplete){
                                                        isResponsed = true
                                                    }
...
//        }
        ...
        Log.d("handleAudioBytes", "$isComplated")
        try {
            if (audioBytes == null || audioBytes!!.isEmpty()) {
                throw Exception()
            }
                    ...
                    geminiApiClient?.sendMessageStream(inputText, object : IListenerStream<String> {
                        override fun onStreamReader(response: String?,flowIndex:Int,lastFlowIndex:Int) {
                            try {
                                ...
                                if (response?.trim() == "###" && jopStreamAudio?.isActive==false) {
                                    cancelJop(callBack)
                                }else{
                                    CoroutineScope(Dispatchers.IO).launch {
                                        try {
